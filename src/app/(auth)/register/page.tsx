import Image from "next/image";
import bannerImg from "../../../../public/images/banner-signin.png";
import googleImg from "../../../../public/images/google-logo.png";

export default function Register() {
    return (
        <div>
            <div>
                <Image
                    src={bannerImg}
                    sizes="100vw"
                    style={{
                        width: '100%',
                        height: 'auto',
                    }}
                    alt="main logo image"
                />
            </div>

            <div className="w-full lg:w-2/5 mx-auto p-5 shadow-lg my-20 py-10 rounded-xl space-y-6">
                <h1 className="text-4xl font-figtree text-[#1A1A1A] font-semibold text-center py-5">Create Account</h1>
                <div>
                    <input type="text" placeholder="Email" className="bg-white w-full border-2 p-3 rounded-xl" />
                </div>
                <div>
                    <input type="text" placeholder="Password" className="bg-white w-full border-2 p-3 rounded-xl" />
                </div>
                <div>
                    <input type="text" placeholder="Confirm Password" className="bg-white w-full border-2 p-3 rounded-xl" />
                </div>
                <div className="flex gap-2 text-[#666666]">
                    <input type="checkbox" className="checkbox" />
                    <p>Accept all terms & Conditions</p>
                </div>
                <button className="text-white bg-[#FF7D29] hover:bg-[#c2570f] w-full p-4 rounded-full font-semibold">Create Account</button>
                <div className="w-full border-2 rounded-xl p-3.5 flex justify-center gap-2">
                    <Image
                        src={googleImg}
                        width={25}
                        height={25}
                        alt="google logo image"
                    />
                    <p className="text-center">Google</p>
                </div>
            </div>
        </div>
    );
}